...
export const GET = async (_req: NextRequest) => {
  ...
  try {
    ...
    for (const camp of campaigns) {
      ...
      const labels = `campaign_id="${camp._id}",campaign_name="${(camp.name ?? '').replace(/"/g, '\\"')}"`;
    ...
    for (const agent of agents) {
      const labels = `agent_id="${agent._id}",agent_name="${(agent.name ?? '').replace(/"/g, '\\"')}",agent_extension="${agent.extension ?? ''}"`;
    ...
    for (const key of queueKeys) {
      ...
      if (metric === 'waiting' || metric === 'active' || metric === 'completed' || metric === 'failed') {
        ...
        lines.push(formatMetric(`dialer_queue_${metric}`, value, { queue: queueName }));
      }
    }
    ...
    for (const key of pacingKeys) {
      ...
      if (metric === 'governor') {
        ...
        if (data) {
          lines.push(formatMetric('dialer_pacing_governor_active', 1, { campaign_id: campaignId, reason: 'abandon_rate' }));
        }
      }
    }
