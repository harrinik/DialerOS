...
interface RetryRule { maxAttempts: number; delayMinutes: number; }
...
export interface ICampaign extends Document {
  ...
  retryRules: { busy: RetryRule; noAnswer: RetryRule; failed: RetryRule; };
  ...
  sipTrunk: string;
  stats: {
    totalContacts: number; dialed: number; answered: number;
    machines: number; failed: number; busy: number;
    noAnswer: number; dnc: number; completed: number; active: number;
  };
  createdAt: Date;
  updatedAt: Date;
}
...
const RetryRuleSchema = new mongoose.Schema<RetryRule>(
  { maxAttempts: { type: Number, default: 3, min: 0, max: 10 }, delayMinutes: { type: Number, default: 5, min: 1 } },
  { _id: false },
);
...
const CampaignSchema = new mongoose.Schema<ICampaign>(
  {
    ...
    dialMode:    { type: String, enum: ['preview', 'progressive', 'predictive'], default: 'progressive' },
    ...
    amdEnabled: { type: Boolean, default: true },    // ISSUE-17: track whether AMD is in use
